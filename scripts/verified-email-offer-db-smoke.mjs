...
import { inspectContactChallenge, verifyContactChallenge } from '../src/workflow/recipient_contacts.mjs';
...
const config = {
  ...
  websiteContactEnrichmentEnabled: false
};
...
try {
  ...
  const foundationBn = '975432198RR0001';
  const recipientBn = '775432198RR0001';
  const publicEmail = `programs-${suffix.toLowerCase()}@recipient.example.ca`;
  const foundation = (await pool.query(`INSERT INTO organizations (business_number,legal_name,organization_type)
    VALUES ($1,$2,'foundation') RETURNING *`, [foundationBn, `Email Foundation ${suffix}`])).rows[0];
  const recipient = (await pool.query(`INSERT INTO organizations (business_number,legal_name,organization_type)
    VALUES ($1,$2,'registered_charity') RETURNING *`, [recipientBn, `Email Recipient ${suffix}`])).rows[0];

  let analyst = await repository.upsertActorFromClaims({ subject:`email-analyst-${suffix}`, email:`analyst-${suffix}@example.ca` });
  let approver = await repository.upsertActorFromClaims({ subject:`email-approver-${suffix}`, email:`approver-${suffix}@example.ca` });
  ...
  analyst = await repository.upsertActorFromClaims({ subject:`email-analyst-${suffix}` });
  approver = await repository.upsertActorFromClaims({ subject:`email-approver-${suffix}` });

  const policy = await createAllocationPolicy(repository, analyst, {
    foundationOrgId: foundation.id,
    title: `Email delivery ${suffix}`,
    ...
    idempotencyKey: `email-policy-${suffix}`
  });
  ...
  const grant = (await pool.query(`INSERT INTO grants
    ...
    RETURNING *`, [foundation.id, recipient.id, policy.id, analyst.id, approver.id, `email-grant-${suffix}`])).rows[0];
  ...
  const fakeT3010 = {
    ...
    charityProfile(bn) {
      ...
      return {
        ...
        sourceYear: 2024,
        website: 'https://recipient.example.ca',
        publicContactCandidates: [
          { channel:'email', destination:publicEmail, sourceKey:'public_email', sourceValue:publicEmail },
  ...
  const contact = (await pool.query(`SELECT * FROM recipient_contacts WHERE organization_id=$1 AND channel='email'`, [recipient.id])).rows[0];
  assert.equal(contact.status, 'verification_pending');
  assert.ok(contact.destination_encrypted);
  assert.equal(contact.destination_encrypted.includes(publicEmail), false);

  const verificationMessages = [];
  const dispatchVerification = await dispatchNotificationsJob({
    config,
    repository,
    provider: { async send(message) { verificationMessages.push(message); return { providerMessageId:'verify-email-1' }; } }
  });
  assert.equal(dispatchVerification.contactVerificationLinks, 1);
  assert.equal(verificationMessages[0].channel, 'email');
  assert.equal(verificationMessages[0].to, publicEmail);
  assert.equal(verificationMessages[0].subject, 'Verify your organization contact channel');
  ...
  const inspected = await inspectContactChallenge(repository, rawToken);
  assert.equal(inspected.channel, 'email');
  assert.equal(inspected.maskedDestination.includes(publicEmail), false);
  assert.match(inspected.maskedDestination, /@recipient\.example\.ca$/);
  ...
  assert.equal(offeredGrant.terms_version, '2026-standard-email-1');

  const offerMessages = [];
  const dispatchOffer = await dispatchNotificationsJob({
    config,
    repository,
    provider: { async send(message) { offerMessages.push(message); return { providerMessageId:'offer-email-1' }; } }
  });
  assert.equal(dispatchOffer.secureOfferLinks, 1);
  assert.equal(offerMessages[0].channel, 'email');
  assert.equal(offerMessages[0].to, publicEmail);
  assert.equal(offerMessages[0].subject, 'Funding offer for your organization');
  ...
  console.log(JSON.stringify({
    ok: true,
    batchId: batch.id,
    emailCandidateSeeded: true,
    publicEmailEncrypted: true,
    emailControlVerified: true,
    grantOfferedAutomaticallyAfterEmailVerification: true,
    secureOfferEmailDelivered: true
  }, null, 2));
} finally {
  await pool.end();
}
