...
test('ingestion normalizes resources and repository searches them', async () => {
  ...
  const fetchImpl = async url => {
    ...
    if (String(url).includes('foundations')) return csvResponse(`BN,Foundation assets,Grant focus\n111111111RR0001,10000000,${'x'.repeat(2_100)} transportation\n333333333RR0001,5000000,\n222222222RR0001,0,\n`);
  ...
  assert.equal(repo.status().financialRecords, 3);
  ...
  const recipientMatch = repo.matchRecipientFoundations({
    profileText: 'Food security and community meals in Ontario',
    requestText: 'Refrigerated truck for food rescue transportation',
    province: 'ON',
    limit: 10,
    minimumSupportSignalCad: 550000
  });
  assert.equal(recipientMatch.screeningOnly, true);
  assert.equal(recipientMatch.matches.length, 1);
  assert.equal(recipientMatch.matches[0].bn, '111111111RR0001');
  assert.equal(recipientMatch.matches[0].supportSignalCad, 600000);
  assert.deepEqual(recipientMatch.matches[0].matchedTerms, ['food', 'transportation']);
  assert.deepEqual(recipientMatch.matches[0].evidence.supportingEvidence[0], {
    sourceKind: 'identification',
    sourceResourceId: 'i',
    sourceUrl: 'https://fixture/ident.csv',
    rowNumber: 1,
    excerpt: 'Public Food Foundation 111111111RR0001 Public Food Foundation ON A',
    matchedTerms: ['food']
  });
  const transportationEvidence = recipientMatch.matches[0].evidence.supportingEvidence[1];
  assert.equal(transportationEvidence.sourceKind, 'foundation_schedule');
  assert.deepEqual(transportationEvidence.matchedTerms, ['transportation']);
  assert.match(transportationEvidence.excerpt, /transportation/i);
  assert.ok(transportationEvidence.excerpt.length <= 1_000);
  assert.match(recipientMatch.matches[0].rationale, /shared filing-evidence terms: food, transportation/i);
  assert.match(recipientMatch.warnings.join(' '), /current guidelines/i);

  const boundedMatch = repo.matchRecipientFoundations({
    profileText: Array.from({ length: 130 }, (_, index) => `recipientterm${index}`).join(' ')
  });
  assert.equal(boundedMatch.queryTerms.length, 100);
});
